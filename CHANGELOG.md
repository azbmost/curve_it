...
- Rearranged the **Other tools** area from three rows of three into two rows of four, tightened its cell padding, and gave the trailing spacer column the grid weight so the buttons stay grouped at the left instead of spreading across the window. The eight launchers are now split into a row that produces or converts a curve file and a row that analyzes one or builds something from it, and the area is narrower than the existing window width, so the window does not grow.
