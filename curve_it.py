...
def read_xyz_curve_from_text(xyz_text: str) -> np.ndarray:
    ...
    - If the first non-empty line is an atom count, treat the file as molecular
      XYZ and skip the first two non-empty/header lines.
...
def write_molecular_xyz_curve(path: str,
                              points: np.ndarray,
                              element: str = "X",
                              comment: Optional[str] = None) -> None:
    """Write curve points as a molecular XYZ file with a dummy atom symbol."""
    pts = np.asarray(points, dtype=float)
    elem = (element or "X").strip() or "X"
    elem = elem[:2]
    if comment is None:
        comment = f"Generated by {APP_NAME} {APP_VERSION}"
    with open(path, "w") as f:
...
def launch_gui() -> None:
    ...
    help_topics = {
        ...
        "kp2xyz": (
            ...
            "Catalogue entries are named exactly the way KnotPlot names them, so ask for 4.1, 6.3.2, or 8.19 the same way you would type load 6.3.2 in KnotPlot. Generator commands work too: enter torus 2 3 for a trefoil built on the fly. Note that KnotPlot's torus p q winds p times about the axis and q times about the tube, and that not every text orders those two numbers the same way, so a (p,q) quoted in a paper may be this tool's torus q p; the knot type is the same either way, the shape is not. List the catalogue to see everything the installed copy provides.\n\n"
            "Preview extracts the current targets into a temporary folder and opens them in Curve It's own 3D curve viewer without touching the output folder, so a knot can be looked at before it is kept; Extract is what writes files. A link comes out as several components separated by blank lines, which Curve It reads as components A, B, C, and so on, so the whole link can be loaded at once and individual components selected afterward. A closed component is written without repeating its first vertex, which is the convention Curve It expects for Path type: closed. Closure is measured per component rather than assumed and is recorded in the # header, so a target that does come back as an open arc, typically a generator command rather than a catalogue entry, is labelled closed=no instead of being passed off as a loop. Resolution is set with the bead count, and on a multi-component link that count is the total across all components, divided between them roughly in proportion to their length.\n\n"
